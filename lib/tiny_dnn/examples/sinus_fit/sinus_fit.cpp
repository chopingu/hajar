...
int main() {
  ...
  auto on_enumerate_epoch = [&]() {
    ...
    std::cerr << "epoch=" << iEpoch << "/" << epochs << " loss=" << loss
              << std::endl;
  };

  // learn
  std::cerr << "learning the sinus function with 2000 epochs:" << std::endl;
  net.fit<tiny_dnn::mse>(opt, X, sinusX, batch_size, epochs, []() {},
                         on_enumerate_epoch, false, 32);
  ...
  for (float x = -3.1416f; x < 3.1416f; x += 0.2f) {
    ...
    if (!first) std::cout << ",";
    std::cout << "\\left(" << x << "," << fPredicted << "\\right)";
    first = false;
    // update max error
  ...
  std::cerr << std::endl << "max_error=" << fMaxError << std::endl;
