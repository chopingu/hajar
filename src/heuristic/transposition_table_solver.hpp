...
struct transposition_table_solver {
    constexpr static usize hash_bytes(std::span<const char> bytes) {
        usize result =
                sizeof(usize) == 8 ? 14695981039346656037ULL : 2166136261U;
    ...
    i32 m_depth = 5;
    lmj::hash_table<gya::compressed_board, heuristic::eval_result,
            compressed_board_hasher>
            m_ttable{};

    [[nodiscard]] eval_result evaluate_board(gya::board const &board,
                                             i32 depth = -1) {
        // lmj::print("hi");
        if (depth == -1) depth = m_depth - 1;
        if (board.has_won().is_game_over()) {
            if (board.has_won().player_1_won())
                return board.turn() == gya::board::PLAYER_ONE ? WINNING_MOVE
                                                              : LOSING_MOVE;
            if (board.has_won().player_2_won())
                return board.turn() == gya::board::PLAYER_TWO ? WINNING_MOVE
                                                              : LOSING_MOVE;
        ...
        for (u8 move: board.get_actions()) {
            eval_result eval =
                    evaluate_board(board.play_copy(move), depth - 1).incremented();
    ...
    [[nodiscard]] u8 operator()(gya::board const &board) {
        for (auto &[b, eval]: m_ttable)
            if (gya::compressed_board::decompress(b).size < board.size)
                m_ttable.erase(b);
        ...
        for (u8 move: actions) {
            const auto eval =
                    evaluate_board(board.play_copy(move)).incremented();
